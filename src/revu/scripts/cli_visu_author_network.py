...
@click.option('--output', default='author_network_dmp.html', show_default=True,
              help='Output file (.html for interactive, .png/.pdf for static)')
@click.option('--static', is_flag=True, default=False,
              help='Produce a static image instead of an interactive HTML')
@click.option('--title', default='Author Co-authorship Map', show_default=True,
              help='Plot title')
def visualize_author_network(embeddings, modeled, metadata, communities, metrics,
                              topic_info, min_papers, top_label_n, output, static, title):
    """Author co-authorship map via DataMapPlot (UMAP-positioned)."""
