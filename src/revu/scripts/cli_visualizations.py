...
def create_visualizations(input_csv, model_path, output_dir, embeddings_2d_path, metadata_csv, label_mode):
    ...
    if label_mode == 'auto':
        ...
        document_labels = [
            "Outlier" if topic == -1 or custom_labels.get(topic, f"Topic {topic}") == "-1"
            else custom_labels.get(topic, f"Topic {topic}")
            for topic in topics
        ]
    ...
    # Filter out outliers 
    filtered_df = plot_df[(plot_df['topic'] != -1) & (plot_df['label'] != 'Outlier')]
